...
MODELNAME = "mnistFashion210607.model"
...
#####modeling#################################################################################
    ...
    dims = [denseNeuronN, learningRate, actFunc, initFunc];
    ...
    parOpt = [optParDict["denseNeutronN"], optParDict["learningRate"],\
              optParDict["activationFunc"], optParDict["initializationFunc"]];
    fitFunc = fitFuncLambda(targetN, inputShape, inputXNorm, inputY,\
                            validationRatio, learnEpochN, dropoutMonteCarloN,\
                            pretrainedLayers=pretrainedLayers);
...
class residualBlock(tf.keras.layers.Layer):
    ...
    _mainLayers = None;
    _skipLayers = None;
    def __init__(self, filterN, strideN=1, actFunc="relu"):
        super(residualBlock, self).__init__();
        self._activation = tf.keras.activations.get(actFunc);
        self._mainLayers = [];
        self._skipLayers = [];
        self._mainLayers.append(tf.keras.layers.Conv2D(filterN, [3, 3], strides=strideN,\
                                                       padding="SAME", use_bias=False));
        self._mainLayers.append(tf.keras.layers.BatchNormalization());
        self._mainLayers.append(self._activation);
        self._mainLayers.append(tf.keras.layers.Conv2D(filterN, [3, 3], strides=1,\
                                                       padding="SAME", use_bias=False));
        self._mainLayers.append(tf.keras.layers.BatchNormalization());
        if strideN > 1:
            self._skipLayers.append(tf.keras.layers.Conv2D(filterN, [1, 1], strides=strideN,\
                                                           padding="SAME", use_bias=False));
            self._skipLayers.append(tf.keras.layers.BatchNormalization());
    #note: Z are tensors (Zout = layer(Zin)): can print out Z.shape
    def call(self, inputs):
        mainZ = inputs + 0;
        for layer in self._mainLayers:
            mainZ = layer(mainZ);
        skipZ = inputs + 0;
        for layer in self._skipLayers:
            skipZ = layer(skipZ);
        mergeZ = mainZ + skipZ; 
        return self._activation(mergeZ);
def buildModel(denseNeuronN, learningRate, actFunc, initFunc,\
               targetN, inputShape, dropoutMCSeed=0, pretrainedLayers=[]):
    #############Adjustables#############
    convLayerNinit  = 64;
    convFilterNinit = (8, 8);
    dropoutRate     = 0.5;
    ...
    if not pretrainedLayers:
        model.add(tf.keras.layers.Conv2D(convLayerNinit, convFilterNinit, strides=2,\
                                         activation=actFunc, padding="SAME",\
                                         input_shape=inputShape)); 
    else:
    ...
    for filterN in [64]*3 + [128]*4 + [256]*6 + [512]*3:
        ...
        model.add(residualBlock(filterN, strideN=strideN));
...
def fitFuncGen(denseNeuronN, learningRate, actFunc, initFunc,\
    ...
    global OPTACCU;
    global OPTASTD;
    parStr = str(denseNeuronN) +"-"+ str(learningRate) +"-"+ str(actFunc) +"-"+ str(initFunc);
    print("##################################################################BEGIN", OPTITER);
    ...
    for dropSeed in range(dropMCN):
        ...
        model=buildModel(denseNeuronN, learningRate, actFunc, initFunc,\
                         targetN, inputShape,\
